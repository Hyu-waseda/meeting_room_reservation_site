...
def submit(user_id):
    ...
    hour = int(tmp[3])
    room = str(tmp[4])
    data = {"title": title, "name": name, "year": year, "month": month,
            "day": day, "hour": hour, "room": room, "user_id": user_id}
    ...
    with env.begin(write=True) as txn:
        id = get_id(txn)
        txn.put(id.encode("utf8"), json.dumps(data).encode("utf8"))
        print(str(data["year"])+"年"+str(data["month"])+"月"+str(data["day"])+"日"+str(
            data["hour"])+"時〜"+str(int(data["hour"])+1)+"時"+str(data["room"])+"の予約を登録")
    return data
    ...
    with env.begin(write=True) as txn:
        ...
        for k, v in cur:
            ...
            if (is_ok_time(d["year"], d["month"], d["day"], d["hour"]) == False):
                txn.delete(k)
                print(str(d["year"])+"年"+str(d["month"])+"月"+str(d["day"])+"日" +
                      str(d["hour"])+"時〜"+str(int(d["hour"])+1)+"時"+str(d["room"])+"の予約を削除")
            else:
                data.append(d)
    data_sorted = sorted(data, key=lambda x: (x["room"], int(
        x["year"]), int(x["month"]), int(x["day"]), int(x["hour"])))
    if user_id == "admin":
        data = [{"user_id": "admin", "user_name": admin_name}]
    else:
        user_data = get_user_data(user_id)
    ...
    with env.begin() as txn:
        ...
        for k, v in cur:
            ...
            if ((name == "" or name == d["name"]) and (month == "" or month == d["month"]) and (day == "" or day == d["day"]) and (room == "" or room == d["room"])):
                tmp.append(d)
    data_sorted = sorted(tmp, key=lambda x: (x["room"], int(
        x["year"]), int(x["month"]), int(x["day"]), int(x["hour"])))
    # data[0]はユーザ名を保持
...
def change_result(user_id):
    ...
    print(title_before, room_before, year_before,
          month_before, day_before, hour_before, name)
    ...
    with env.begin(write=True) as txn:
        ...
        for k, v in cur:
            ...
            if (d["name"] == name and d["room"] == room_before and d["year"] == year_before and d["month"] == month_before and d["day"] == day_before and d["hour"] == hour_before):
                ...
                print(str(d["year"])+"年"+str(d["month"])+"月"+str(d["day"])+"日" +
                      str(d["hour"])+"時〜"+str(int(d["hour"])+1)+"時"+str(d["room"])+"の予約を削除")
                data = {"title_before": title_before, "room_before": room_before, "year_before": year_before,
                        "month_before": month_before, "day_before": day_before, "hour_before": hour_before}

    # 変更後の予約
    data_reserve = {"title": title, "name": name, "year": year, "month": month,
                    "day": day, "hour": hour, "room": room, "user_id": user_id}
    data.update({"title": title, "name": name, "year": year, "month": month,
                "day": day, "hour": hour, "room": room, "user_id": user_id})
    if (is_ok_room(year, month, day, hour, room) == False):
        return bottle.template("./tpl/error_submit_overlapping_time", user_id=user_id)
    elif (is_ok_time(year, month, day, hour) == False):
